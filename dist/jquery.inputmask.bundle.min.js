...
g.oncleared);H(B,!0,!1);var Z;try{Z=document.activeElement}catch(ga){}Z===B?(z.addClass("focus.inputmask"),v(B,c().p)):g.clearMaskOnLostFocus&&(l().join("")==m().join("")?B._valueSet(""):V(B));R(B)}};return this}var g=f.extend(!0,{},f.inputmask.defaults,d),F=null!==navigator.userAgent.match(/msie 10/i),ea=null!==navigator.userAgent.match(/iphone/i),S=null!==navigator.userAgent.match(/android.*safari.*/i),fa=function(a){var b=document.createElement("input"),a="on"+a,c=a in b;c||(b.setAttribute(a,"return;"),
c="function"==typeof b[a]);return c}("paste")&&!F?"paste":"input",aa,A,w=0;S&&(F=navigator.userAgent.match(/safari.*/i),aa=533>=parseInt(RegExp(/[0-9]+/).exec(F)));if("string"===typeof a)switch(a){case "mask":return e(g.alias,d),A=h(),this.each(function(){j(f.extend(true,{},A),0).mask(this)});case "unmaskedvalue":return F=f(this),A=F.data("_inputmask").masksets,w=F.data("_inputmask").activeMasksetIndex,g=F.data("_inputmask").opts,j(A,w).unmaskedvalue(this);case "remove":return this.each(function(){var a=
