...
if(document.activeElement===d){var b=K(d);if(b.begin===b.end)switch(g.positionCaretOnClick){case"none":break;case"radixFocus":if(c(b.begin)){K(d,g.numericInput?D(a.inArray(g.radixPoint,x())):a.inArray(g.radixPoint,x()));break}default://lvp:
var e=b.begin,f=o(e,!0),h=D(f);if(e<h)K(d,C(e)||C(e-1)?e:D(e));else{var i=H(h);(""!==i&&x()[h]!==i&&s(h).match.optionalQuantifier!==!0||!C(h,!0)&&s(h).match.def===i)&&(h=D(h)),K(d,h)}}}},0)}function Y(a){var b=this;setTimeout(function(){K(b,0,D(o()))},0)}function Z(c){var d=this,e=a(d),f=K(d),h=c.originalEvent||c,i=window.clipboardData||h.clipboardData,j=ia?x().slice(f.end,f.begin):x().slice(f.begin,f.end);i.setData("text",ia?j.reverse().join(""):j.join("")),document.execCommand&&document.execCommand("copy"),// copy selected content to system clipbaord
