...
!function webpackUniversalModuleDefinition(root, factory) {
	...
	return function (modules) {
		...
		var $ = __webpack_require__(3), window = __webpack_require__(4), document = window.document,
							...
							for (var nmsp in eventRegistry[evNdx]) {
								if (nmsp === namespace) {
									if (void 0 === handler) {
										...
										evts.push({ev: evNdx, namespace: nmsp, handler: handler});
									}
								}
							}
						}
				...
				if (isValidElement(this[0])) {
					for (var eventRegistry = this[0].eventRegistry, elem = this[0], _events = "string" == typeof events ? events.split(" ") : [events.type], endx = 0; endx < _events.length; endx++) {
						...
						if (void 0 !== document && "global" === namespace) {
							var evnt, i, params = {bubbles: !0, cancelable: !0, detail: arguments[1]};
							if (document.createEvent) {
								...
								evnt = document.createEventObject(), evnt.eventType = ev, evnt.detail = arguments[1], events.type && DependencyLib.extend(evnt, events), elem.fireEvent("on" + evnt.eventType, evnt);
							}
						} else if (void 0 !== eventRegistry[ev]) if (events = events.type ? events : DependencyLib.Event(events), "global" === namespace) for (var nmsp in eventRegistry[ev]) for (i = 0; i < eventRegistry[ev][nmsp].length; i++) eventRegistry[ev][nmsp][i].apply(elem, arguments); else for (i = 0; i < eventRegistry[ev][namespace].length; i++) eventRegistry[ev][namespace][i].apply(elem, arguments);
					}
				}
				return this;
			}
		...
		module.exports = function (actionObj, maskset, opts) {
			...
			function getMaskTemplate(baseOnInput, minimalPos, includeMode, noJit, clearOptionalTail) {
				...
				do {
					if (!0 === baseOnInput && getMaskSet().validPositions[pos]) {
						testPos = clearOptionalTail && !0 === getMaskSet().validPositions[pos].match.optionality && void 0 === getMaskSet().validPositions[pos + 1] && (!0 === getMaskSet().validPositions[pos].generatedInput || getMaskSet().validPositions[pos].input == opts.skipOptionalPartCharacter && 0 < pos) ? determineTestTemplate(pos, getTests(pos, ndxIntlzr, pos - 1)) : getMaskSet().validPositions[pos], test = testPos.match, ndxIntlzr = testPos.locator.slice(), maskTemplate.push(!0 === includeMode ? testPos.input : !1 === includeMode ? test.nativeDef : getPlaceholder(pos, test));
					} else {
			...
			function getTests(pos, ndxIntlzr, tstPs) {
					...
					function handleMatch(match, loopNdx, quantifierRecurse) {
						...
						function isSubsetOf(source, target) {
							function expand(pattern) {
								for (var expanded = [], start, end, i = 0, l = pattern.length; i < l; i++) if ("-" === pattern.charAt(i)) for (end = pattern.charCodeAt(i + 1); ++start < end;) expanded.push(String.fromCharCode(start)); else start = pattern.charCodeAt(i), expanded.push(pattern.charAt(i));
						...
						function staticCanMatchDefinition(source, target) {
							...
							return canMatch = !(!canMatch || null !== source.match.fn || null === target.match.fn) && target.match.fn.test(source.match.def, getMaskSet(), pos, !1, opts, !1), canMatch;
						}
				...
				function mergeLocators(pos, tests) {
					var locator = [];
					return $.isArray(tests) || (tests = [tests]), 0 < tests.length && (void 0 === tests[0].alternation ? (locator = determineTestTemplate(pos, tests.slice()).locator.slice(), 0 === locator.length && (locator = tests[0].locator.slice())) : $.each(tests, function (ndx, tst) {
				...
				}), void 0 !== ndxIntlzr && getMaskSet().tests[pos] ? $.extend(!0, [], matches) : (getMaskSet().tests[pos] = $.extend(!0, [], matches), getMaskSet().tests[pos]);
			}
			...
			function alternate(pos, c, strict, fromIsValid, rAltPos) {
				...
				if (void 0 !== alternation) {
					...
					for (i = decisionPos; i < getLastValidPosition(void 0, !0) + 1; i++) validPos = getMaskSet().validPositions[i], validPos && !0 !== validPos.generatedInput ? validInputsClone.push(validPos.input) : i < pos && staticInputsBeforePos++, delete getMaskSet().validPositions[i];
					for (; getMaskSet().excludes[decisionPos] && getMaskSet().excludes[decisionPos].length < 10;) {
						...
						if (isValidRslt && void 0 !== c) {
							...
							for (i = decisionPos; i < getLastValidPosition() + 1; i++) validPos = getMaskSet().validPositions[i], (void 0 === validPos || null == validPos.match.fn) && i < pos + posOffset && posOffset++;
			...
			function isValid(pos, c, strict, fromIsValid, fromAlternate, validateOnly) {
				...
				function processCommandObject(commandObj) {
					if (void 0 !== commandObj && (void 0 !== commandObj.remove && ($.isArray(commandObj.remove) || (commandObj.remove = [commandObj.remove]), $.each(commandObj.remove.sort(function (a, b) {
						return b - a;
					}), function (ndx, lmnt) {
						revalidateMask({begin: lmnt, end: lmnt + 1});
					})), void 0 !== commandObj.insert && ($.isArray(commandObj.insert) || (commandObj.insert = [commandObj.insert]), $.each(commandObj.insert.sort(function (a, b) {
						return a - b;
					}), function (ndx, lmnt) {
				...
				function _isValid(position, c, strict) {
					...
					return $.each(getTests(position), function (ndx, tst) {
						...
						if (getBuffer(!0), rslt = null != test.fn ? test.fn.test(c, getMaskSet(), position, strict, opts, isSelection(pos)) : (c === test.def || c === opts.skipOptionalPartCharacter) && "" !== test.def && {
							...
							return elem = elem === opts.skipOptionalPartCharacter && null === test.fn ? getPlaceholder(position, test, !0) || test.def : elem, rslt = processCommandObject(rslt), !0 !== rslt && void 0 !== rslt.pos && rslt.pos !== position && (validatedPos = rslt.pos), !0 !== rslt && void 0 === rslt.pos && void 0 === rslt.c ? !1 : (revalidateMask(pos, $.extend({}, tst, {input: casing(elem, test, validatedPos)}), fromIsValid, validatedPos) || (rslt = !1), !1);
						}
					}), rslt;
				}
				...
				if ($.isFunction(opts.preValidation) && !strict && !0 !== fromIsValid && !0 !== validateOnly && (result = opts.preValidation(getBuffer(), maskPos, c, isSelection(pos), opts, getMaskSet())), !0 === result) {
					if ((void 0 === maxLength || maskPos < maxLength) && (result = _isValid(maskPos, c, strict), (!strict || !0 === fromIsValid) && !1 === result && !0 !== validateOnly)) {
						...
						if (!currentPosValid || null !== currentPosValid.match.fn || currentPosValid.match.def !== c && c !== opts.skipOptionalPartCharacter) {
							if ((opts.insertMode || void 0 === getMaskSet().validPositions[seekNext(maskPos)]) && (getMaskSet().jitOffset[maskPos] && void 0 === getMaskSet().validPositions[seekNext(maskPos)] && (result = isValid(maskPos + getMaskSet().jitOffset[maskPos], c, !0), !1 !== result && (result.caret = maskPos)), !isMask(maskPos, !0))) {
				...
				for (var ps = originalPos; ps < newPos; ps++) {
					if (void 0 === getMaskSet().validPositions[ps] && !isMask(ps, !0)) {
						...
						if (vp) {
							...
							if (bestMatch && (!0 !== bestMatch.match.jit || "master" === bestMatch.match.newBlockMarker && (np = getMaskSet().validPositions[ps + 1]) && !0 === np.match.optionalQuantifier) && (bestMatch = $.extend({}, bestMatch, {input: getPlaceholder(ps, bestMatch.match, !0) || bestMatch.match.def}), bestMatch.generatedInput = !0, revalidateMask(ps, bestMatch, !0), !0 !== fillOnly)) {
			...
			function revalidateMask(pos, validTest, fromIsValid, validatedPos) {
				...
				if (pos.begin > pos.end && (begin = pos.end, end = pos.begin), validatedPos = void 0 !== validatedPos ? validatedPos : begin, begin !== end || opts.insertMode && void 0 !== getMaskSet().validPositions[validatedPos] && void 0 === fromIsValid) {
					...
					for (validTest && (getMaskSet().validPositions[validatedPos] = $.extend(!0, {}, validTest), posMatch++, j++, begin < end && i++); i <= lvp; i++) {
						...
						if (void 0 !== t && (end <= i || begin <= i && !0 !== t.generatedInput && IsEnclosedStatic(i, positionsClone, {
							...
							for (; "" !== getTest(posMatch).match.def;) {
								if (!1 === needsValidation && positionsClone[posMatch] && positionsClone[posMatch].match.nativeDef === t.match.nativeDef) {
									getMaskSet().validPositions[posMatch] = $.extend(!0, {}, positionsClone[posMatch]), getMaskSet().validPositions[posMatch].input = t.input, trackbackPositions(void 0, posMatch, !0), j = posMatch + 1, valid = !0;
								} else if (opts.shiftPositions && positionCanMatchDefinition(posMatch, t.match.def)) {
									var result = isValid(posMatch, t.input, !0, !0);
									valid = !1 !== result, j = (result.pos || posMatch) + 1, needsValidation = !0;
								} else {
									valid = !0 === t.generatedInput || t.input === opts.radixPoint && !0 === opts.numericInput;
								}
								if (valid) break;
					...
					validTest && (getMaskSet().validPositions[validatedPos] = $.extend(!0, {}, validTest));
				}
			...
			function getPlaceholder(pos, test, returnPL) {
				...
				if (-1 < pos && void 0 === getMaskSet().validPositions[pos]) {
					...
					if (tests.length > 1 + ("" === tests[tests.length - 1].match.def ? 1 : 0)) for (var i = 0; i < tests.length; i++) if (!0 !== tests[i].match.optionality && !0 !== tests[i].match.optionalQuantifier && (null === tests[i].match.fn || void 0 === prevTest || !1 !== tests[i].match.fn.test(prevTest.match.def, getMaskSet(), pos, !0, opts)) && (staticAlternations.push(tests[i]), null === tests[i].match.fn && (prevTest = tests[i]), 1 < staticAlternations.length && /[0-9a-bA-Z]/.test(staticAlternations[0].match.def))) return opts.placeholder.charAt(pos % opts.placeholder.length);
				}
				return test.def;
			}
				...
				keydownEvent: function keydownEvent(e) {
					...
					function hanndleRadixDance(pos, c) {
						if (opts._radixDance && opts.numericInput) {
							...
							pos.begin <= radixPos && (0 < radixPos || c == opts.radixPoint) && (c == opts.radixPoint && 0 < radixPos && (offset = 1), pos.begin -= 1, pos.end -= 1);
						}
						return pos;
					}

					if (!(!0 === checkval || e.ctrlKey && e.altKey) && (e.ctrlKey || e.metaKey || ignorable)) {
						return k === Inputmask.keyCode.ENTER && undoValue !== getBuffer().join("") && (undoValue = getBuffer().join(""), setTimeout(function () {
					...
					var input = this, ev = e.originalEvent || e, $input = $(this),
						inputValue = this.inputmask._valueGet(!0), caretPos = caret(this), tempValue;
					isRTL && (tempValue = caretPos.end, caretPos.end = caretPos.begin, caretPos.begin = tempValue);
					var valueBeforeCaret = inputValue.substr(0, caretPos.begin),
						valueAfterCaret = inputValue.substr(caretPos.end, inputValue.length);
					if (valueBeforeCaret === (isRTL ? getBufferTemplate().reverse() : getBufferTemplate()).slice(0, caretPos.begin).join("") && (valueBeforeCaret = ""), valueAfterCaret === (isRTL ? getBufferTemplate().reverse() : getBufferTemplate()).slice(caretPos.end).join("") && (valueAfterCaret = ""), window.clipboardData && window.clipboardData.getData) {
								...
								switch (opts.positionCaretOnClick) {
									...
									default:
										...
										if (clickPosition < lastPosition) {
											caret(input, isMask(clickPosition, !0) || isMask(clickPosition - 1, !0) ? clickPosition : seekNext(clickPosition));
										} else {
					...
					var input = this, $input = $(this), pos = caret(this), ev = e.originalEvent || e,
			...
			function checkVal(input, writeOut, strict, nptvl, initiatingEvent) {
				...
				-1 === initialNdx ? (getMaskSet().p = seekNext(initialNdx), initialNdx = 0) : getMaskSet().p = initialNdx, inputmask.caretPos = {begin: initialNdx};
				var staticMatches = [], sndx, validPos, nextValid;
				if ($.each(inputValue, function (ndx, charCode) {
					if (void 0 !== charCode) {
						if (void 0 === getMaskSet().validPositions[ndx] && inputValue[ndx] === getPlaceholder(ndx) && isMask(ndx, !0) && !1 === isValid(ndx, inputValue[ndx], !0, void 0, void 0, !0)) {
							getMaskSet().p++;
						} else {
							var keypress = new $.Event("_checkval");
							keypress.which = charCode.charCodeAt(0), charCodes += charCode;
							var lvp = getLastValidPosition(void 0, !0);
							isTemplateMatch(initialNdx, charCodes) ? result = EventHandlers.keypressEvent.call(input, keypress, !0, !1, strict, lvp + 1) : (result = EventHandlers.keypressEvent.call(input, keypress, !0, !1, strict, inputmask.caretPos.begin), result && (initialNdx = inputmask.caretPos.begin + 1, charCodes = "")), result && (result.pos && getMaskSet().validPositions[result.pos] && null === getMaskSet().validPositions[result.pos].match.fn && (staticMatches.push(result.pos), result.forwardPosition = result.pos + 1), writeBuffer(void 0, getBuffer(), result.forwardPosition, keypress, !1), inputmask.caretPos = {
					...
					if (isComplete(getBuffer())) {
						for (; sndx = staticMatches.pop();) validPos = getMaskSet().validPositions[sndx], validPos.generatedInput = !0;
					} else {
						for (; sndx = staticMatches.pop();) {
							var keypress = new $.Event("_checkval"), nextSndx = sndx + 1;
							for (validPos = getMaskSet().validPositions[sndx], validPos.generatedInput = !0, keypress.which = validPos.input.charCodeAt(0); (nextValid = getMaskSet().validPositions[nextSndx]) && nextValid.input === validPos.input;) nextSndx++;
			...
			function clearOptionalTail(buffer) {
				buffer.length = 0;
				for (var template = getMaskTemplate(!0, 0, !0, void 0, !0), lmnt, validPos; lmnt = template.shift(), void 0 !== lmnt;) buffer.push(lmnt);
				return buffer;
			}
			...
			function handleRemove(input, k, pos, strict, fromIsValid) {
				...
				if (lvp < pos.begin || -1 === pos.begin) getMaskSet().p = seekNext(lvp); else if (!0 !== strict && (getMaskSet().p = pos.begin, !0 !== fromIsValid)) for (; getMaskSet().p < lvp && void 0 === getMaskSet().validPositions[getMaskSet().p];) getMaskSet().p++;
			}
			...
			function renderColorMask(input, caretPos, clear) {
				...
				function setEntry(entry) {
					if (void 0 === entry && (entry = ""), isStatic || null !== test.fn && void 0 !== testPos.input) {
						if (isStatic && (null !== test.fn && void 0 !== testPos.input || "" === test.def)) {
							isStatic = !1;
						...
						isStatic = !0, maskTemplate.push("<span class='im-static'>" + entry);
					}
				}
				...
				if (void 0 !== colorMask) {
					...
					if (void 0 === caretPos ? caretPos = caret(input) : void 0 === caretPos.begin && (caretPos = {
						...
						for (var lvp = getLastValidPosition(); getMaskSet().validPositions[pos] ? (testPos = getMaskSet().validPositions[pos], test = testPos.match, ndxIntlzr = testPos.locator.slice(), setEntry(buffer[pos])) : (testPos = getTestTemplate(pos, ndxIntlzr, pos - 1), test = testPos.match, ndxIntlzr = testPos.locator.slice(), !1 === opts.jitMasking || pos < lvp || "number" == typeof opts.jitMasking && isFinite(opts.jitMasking) && opts.jitMasking > pos ? setEntry(getPlaceholder(pos, test)) : isStatic = !1), pos++, (void 0 === maxLength || pos < maxLength) && (null !== test.fn || "" !== test.def) || pos < lvp || isStatic;) ;
			...
			function applyInputValue(input, value) {
				input.inputmask.refreshValue = !1, $.isFunction(opts.onBeforeMask) && (value = opts.onBeforeMask.call(inputmask, value, opts) || value), value = value.split(""), checkVal(input, !0, !1, value), undoValue = getBuffer().join(""), (opts.clearMaskOnLostFocus || opts.clearIncomplete) && input.inputmask._valueGet() === getBufferTemplate().join("") && input.inputmask._valueSet("");
			}
							...
							if ($.valHooks && (void 0 === $.valHooks[type] || !0 !== $.valHooks[type].inputmaskpatch)) {
								...
								$.valHooks[type] = {
									get: function get(elem) {
										...
										return result = valhookSet(elem, value), elem.inputmask && applyInputValue(elem, value), result;
									}, inputmaskpatch: !0
								};
							}
						}
			...
			for (var mask = "", match; match = getTokenizer(opts).exec(format);) {
				if (void 0 === dateObjValue) {
					...
					if (!0 !== raw && formatCode[match[0]][3]) {
						...
						formatCode[match[0]][2] ? mask += dateObjValue["raw" + formatCode[match[0]][2]] : mask += match[0];
					}
				} else {
					mask += match[0];
				}
		...
		Inputmask.extendAliases({
			numeric: {
				mask: function mask(opts) {
					...
					var mask = "[+]", altMask;
					if (mask += autoEscape(opts.prefix, opts), "" !== opts.groupSeparator ? mask += "(" + opts.groupSeparator + "999){+|1}" : mask += "9{+}", void 0 !== opts.digits) {
					...
					return mask += autoEscape(opts.suffix, opts), mask += "[-]", altMask && (mask = altMask + autoEscape(opts.suffix, opts) + "[-]"), opts.greedy = !1, console.log(mask), mask;
				},
				placeholder: "0",
				greedy: !1,
				digits: "*",
				...
				preValidation: function preValidation(buffer, pos, c, isSelection, opts, maskset) {
					...
					if ("-" !== c && c !== opts.negationSymbol.front) return -1 === radixPos || !0 !== opts._radixDance || !1 !== isSelection || c !== opts.radixPoint || void 0 === opts.digits || !(isNaN(opts.digits) || 0 < parseInt(opts.digits)) || radixPos === pos || {caret: opts._radixDance && radixPos < pos ? radixPos : radixPos - 1};
					...
					return $.each(maskset.validPositions, function (ndx, tst) {
						...
						insert: {pos: parseInt(GetLastValidPosition(maskset)) + 1, c: c, fromIsValid: !0},
						caret: pos <= radixPos ? pos + 1 : pos
					};
				},
				postValidation: function postValidation(buffer, pos, currentResult, opts) {
				...
				onBeforeWrite: function onBeforeWrite(e, buffer, caretPos, opts) {
					...
					if (e) {
						switch (e.type) {
							...
							case"checkval":
								var unmasked;
								parseMinMaxOptions(opts), null === opts.min && null === opts.max || (unmasked = opts.onUnMask(buffer.join(""), void 0, $.extend({}, opts, {unmaskAsNumber: !0})), null !== opts.min && unmasked < opts.min ? result = {
									refreshFromBuffer: !0,
			...
			indianns: {
				alias: "numeric", mask: function mask(opts) {
					opts.repeat = 0, opts.groupSeparator = ",", opts.radixPoint = ".", "radixFocus" === opts.positionCaretOnClick && "" === opts.placeholder && (opts.positionCaretOnClick = "lvp"), !0 === opts.numericInput ? (opts.positionCaretOnClick = "radixFocus" === opts.positionCaretOnClick ? "lvp" : opts.positionCaretOnClick, opts.digitsOptional = !1, isNaN(opts.digits) && (opts.digits = 2), opts._radixDance = !1) : opts.numericInput = !0;
					var mask = "[+]";
					if (mask += autoEscape(opts.prefix, opts), mask += "(" + opts.groupSeparator + "99){*|1}(" + opts.groupSeparator + "999){1|1}", void 0 !== opts.digits) {
						var dq = opts.digits.toString().split(",");
					...
					return mask += autoEscape(opts.suffix, opts), mask += "[-]", opts.greedy = !1, console.log(mask), mask
				}, placeholder: "0", digits: 2, digitsOptional: !1
			}
		}), module.exports = Inputmask
	}])
});