...
module.exports = function maskScope(actionObj, maskset, opts) {
  ...
  ignorable = false,
      ...
      colorMask,
      originalPlaceholder,
      focusDelay = 0; //maskset helperfunctions
      ...
      if (pos > -1 && getMaskSet().validPositions[pos] === undefined) {
        ...
        if (tests.length > 1 + (tests[tests.length - 1].match.def === "" ? 1 : 0)) {
          for (var i = 0; i < tests.length; i++) {
            if (tests[i].match.optionality !== true && tests[i].match.optionalQuantifier !== true && (tests[i].match["static"] === true || prevTest === undefined || tests[i].match.fn.test(prevTest.match.def, getMaskSet(), pos, true, opts) !== false)) {
...
Inputmask.extendAliases({
  "numeric": {
    ...
    onBeforeWrite: function onBeforeWrite(e, buffer, caretPos, opts) {
      //check leading zeros
      var numberMatches = new RegExp("^" + opts.negationSymbol.front + "?" + Inputmask.escapeRegex(opts.prefix) + "(?<number>.*)" + Inputmask.escapeRegex(opts.suffix) + opts.negationSymbol.back + "?" + "$").exec(buffer.slice().reverse().join("")),
          number = numberMatches ? numberMatches.groups.number : "";
      number = number.split(opts.radixPoint.charAt(0))[0];
      var leadingzeroes = new RegExp("^[0" + opts.groupSeparator + "]*").exec(number);

      if (leadingzeroes[0].length > 1) {
