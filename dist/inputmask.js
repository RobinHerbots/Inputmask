...
module.exports = function maskScope(actionObj, maskset, opts) {
  ...
  function positionCanMatchDefinition(pos, testDefinition, opts) {
    ...
    for (var tndx = 0; tndx < tests.length; tndx++) {
      if (tests[tndx].match && (tests[tndx].match["nativeDef"] === testDefinition.match[defProp] || tests[tndx].match["nativeDef"] === testDefinition.match["nativeDef"])) {
        valid = true;
        break;
      }
    }
        ...
        if ((t = positionsClone[i]) !== undefined && t.generatedInput !== true && (i >= end || i >= begin && IsEnclosedStatic(i, positionsClone, {
          ...
          while (getTest(posMatch).match.def !== "") {
            ...
            if (positionCanMatchDefinition(posMatch, t, opts) || t.match.def === "+") {
              ...
              valid = result !== false;
              j = (result.pos || posMatch) + 1; //if (!valid) break;
            } else {
              valid = false;
            }
