...
Inputmask.prototype = {
  ...
  defaults: {
    ...
    preValidation: null,
    //hook to preValidate the input.  Usefull for validating regardless the definition.	args => buffer, pos, char, isSelection, opts, maskset, caretPos => return true/false/command object
    postValidation: null,
...
module.exports = function maskScope(actionObj, maskset, opts) {
    ...
    function resolveTestFromToken(maskToken, ndxInitializer, loopNdx, quantifierRecurse) {
      ...
      function handleMatch(match, loopNdx, quantifierRecurse) {
        ...
        function isSubsetOf(source, target) {
          ...
          if (opts.regex && source.match["static"] !== true && target.match["static"] !== true) {
            //is regex a subset
      ...
      if (maxLength === undefined || maskPos < maxLength) {
        ...
        if ((!strict || fromIsValid === true) && result === false && validateOnly !== true) {
          ...
          if (currentPosValid && currentPosValid.match["static"] === true && (currentPosValid.match.def === c || c === opts.skipOptionalPartCharacter)) {
            ...
            if (opts.insertMode || maskset.validPositions[seekNext(maskPos)] === undefined) {
  ...
  function revalidateMask(pos, validTest, fromIsValid, validatedPos) {
    ...
    if (begin !== end || opts.insertMode && maskset.validPositions[validatedPos] !== undefined && fromIsValid === undefined) {
      ...
      var valid = true,
          j = validatedPos,
          needsValidation = validatedPos === 0,
          posMatch = j;
      i = j;
        ...
        if (t !== undefined && (i >= end || i >= begin && t.generatedInput !== true && IsEnclosedStatic(i, positionsClone, {
          ...
          while (getTest(posMatch).match.def !== "") {
            ...
            if (needsValidation === false && positionsClone[posMatch] && positionsClone[posMatch].match.nativeDef === t.match.nativeDef) {
              ...
              j = posMatch + 1;
              valid = true; //we still need some hackery for the + validator (numeric alias)
            } else if (opts.shiftPositions && (positionCanMatchDefinition(posMatch, t.match.def) || t.match.def === "+")) {
              //validated match
  ...
  function seekPrevious(pos, newBlock) {
    ...
    while (--position > 0 && (newBlock === true && getTest(position).match.newBlockMarker !== true || newBlock !== true && !isMask(position) && ( // eslint-disable-next-line no-empty
  ...
  function handleRemove(input, k, pos, strict, fromIsValid) {
    ...
    if (k === Inputmask.keyCode.BACKSPACE && pos.end - pos.begin < 1) {
      ...
      pos.end = isMask(pos.end, true) && maskset.validPositions[pos.end] && maskset.validPositions[pos.end].input !== opts.radixPoint ? pos.end + 1 : seekNext(pos.end) + 1;
  ...
  function renderColorMask(input, caretPos, clear) {
    ...
    function setCaret() {
      if (document.activeElement === input) {
        maskTemplate.splice(caretPos.begin, 0, caretPos.begin === caretPos.end || caretPos.end > maskset.maskLength ? "<mark class=\"im-caret\" style=\"border-right-width: 1px;border-right-style: solid;\">" : "<mark class=\"im-caret-select\">");
