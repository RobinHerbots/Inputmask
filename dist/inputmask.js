...
iemobile = /iemobile/i.test(ua),
    ...
    keyCode = __webpack_require__(8); //masking scope
...
module.exports = function maskScope(actionObj) {
  var inputmask = this,
      ...
      opts = inputmask.opts,
      el = inputmask.el,
      isRTL = inputmask.isRTL || (inputmask.isRTL = opts.numericInput); //maskset helperfunctions
  ...
  function isValid(pos, c, strict, fromIsValid, fromAlternate, validateOnly, fromCheckval) {
    ...
    if (result === true) {
      ...
      if (result === false && opts.keepStatic && (isComplete(getBuffer()) || maskPos === 0) && !strict && fromAlternate !== true) {
    ...
    if ($.isFunction(opts.postValidation) && fromIsValid !== true && validateOnly !== true) {
      var postResult = opts.postValidation.call(el, getBuffer(true), pos.begin !== undefined ? isRTL ? pos.end : pos.begin : pos, c, result, opts, maskset, strict, fromCheckval);
      ...
      for (i = validTest ? end : end - 1; i <= lvp; i++) {
        if ((t = positionsClone[i]) !== undefined && t.generatedInput !== true && (i >= end || i >= begin && IsEnclosedStatic(i, positionsClone, {
          ...
          while (getTest(posMatch).match.def !== "") {
            ...
            if ((canMatch = positionCanMatchDefinition(posMatch, t, opts)) !== false || t.match.def === "+") {
  ...
  function seekNext(pos, newBlock, fuzzy) {
    ...
    while (getTest(position).match.def !== "" && (newBlock === true && (getTest(position).match.newBlockMarker !== true || !isMask(position, undefined, true)) || newBlock !== true && !isMask(position, undefined, fuzzy))) {
      position++;
    }
  ...
  var EventHandlers = {
    keydownEvent: function keydownEvent(e) {
      ...
      inputmask.ignorable = $.inArray(k, opts.ignorables) !== -1;
    },
    keypressEvent: function keypressEvent(e, checkval, writeOut, strict, ndx) {
      ...
      if (checkval !== true && !(e.ctrlKey && e.altKey) && (e.ctrlKey || e.metaKey || inputmask.ignorable)) {
        if (k === keyCode.ENTER && inputmask.undoValue !== getBuffer().join("")) {
          inputmask.undoValue = getBuffer().join(""); // e.preventDefault();
        ...
        var valResult = isValid(pos, c, strict, undefined, undefined, undefined, checkval);
    ...
    focusEvent: function focusEvent(e) {
      ...
      if (opts.positionCaretOnTab === true && inputmask.mouseEnter === false && (!isComplete(getBuffer()) || getLastValidPosition() === -1)) {
    ...
    invalidEvent: function invalidEvent(e) {
      inputmask.validationEvent = true;
    },
    ...
    cutEvent: function cutEvent(e) {
      ...
      writeBuffer(input, getBuffer(), maskset.p, e, inputmask.undoValue !== getBuffer().join(""));
    },
    blurEvent: function blurEvent(e) {
      ...
      if (input.inputmask) {
        ...
        if (inputmask.undoValue !== getBuffer().join("")) {
          inputmask.undoValue = getBuffer().join("");
          $input.trigger("change");
        }
      }
  ...
  function checkVal(input, writeOut, strict, nptvl, initiatingEvent) {
    ...
    if (staticMatches.length > 0) {
      var sndx,
          validPos,
          nextValid = seekNext(-1, undefined, false);

      if (!isComplete(getBuffer()) && staticMatches.length <= nextValid || isComplete(getBuffer()) && staticMatches.length > 0 && staticMatches.length !== nextValid && staticMatches[0] === 0) {
    ...
    if (writeOut) {
      writeBuffer(input, getBuffer(), result ? result.forwardPosition : inputmask.caretPos.begin, initiatingEvent || new $.Event("checkval"), initiatingEvent && initiatingEvent.type === "input" && inputmask.undoValue !== getBuffer().join("")); // for (var vndx in maskset.validPositions) {
  ...
  function handleRemove(input, k, pos, strict, fromIsValid) {
    ...
    if (pos.end >= getBuffer().length && lvp >= pos.end) {
  ...
  function mask() {
    function isElementTypeSupported(input, opts) {
      function patchValueProperty(npt) {
        ...
        function getter() {
          if (this.inputmask) {
            return this.inputmask.opts.autoUnmask ? this.inputmask.unmaskedvalue() : getLastValidPosition() !== -1 || opts.nullable !== true ? (this.inputmask.shadowRoot || document.activeElement) === this && opts.clearMaskOnLostFocus ? (isRTL ? clearOptionalTail(getBuffer().slice()).reverse() : clearOptionalTail(getBuffer().slice())).join("") : valueGet.call(this) : "";
          } else {
    ...
    if (isSupported !== false) {
      ...
      inputmask.undoValue = getBufferTemplate().join(""); //initialize the buffer and getmasklength
...
var Inputmask = __webpack_require__(2),
    ...
    keyCode = __webpack_require__(8),
    currentYear = new Date().getFullYear(),
    escapeRegex = __webpack_require__(10)["default"],
...
function isValidDate(dateParts, currentResult, opts) {
  if (!isFinite(dateParts.rawday) || dateParts.day == "29" && !isFinite(dateParts.rawyear) || new Date(dateParts.date.getFullYear(), isFinite(dateParts.rawmonth) ? dateParts.month : dateParts.date.getMonth() + 1, 0).getDate() >= dateParts.day) {
    return currentResult;
  } else {
    ...
    if (dateParts["rawyear"]) {
      ...
      if (rawYear < minYear) {
        ...
        if (minYear <= rawYear) {
          ...
          if (rawYear.length === 1 && minYear <= rawYear <= maxYear && fromCheckval !== true) {
            //this can match
            if (tokenMatch.targetMatch[0] === "yyyy") {
              result.insert = [{
                pos: result.pos + 1,
      ...
      if (e) {
        switch (e.type) {
          ...
          case "checkval":
            ...
            } else if (opts.radixPoint !== "" && buffer[0] === opts.radixPoint) {
