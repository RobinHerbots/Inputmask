    ...
    while (
      ...
      for (i = 0; i < validInputs.length; i++) {
        ...
        if (
          ...
          nextPos = determineNewCaretPosition.call(
            inputmask,
            {
              begin: nextPos,
              end: nextPos
...
function handleRemove(input, c, pos, strict, fromIsValid) {
  ...
  if (
    ...
    pos.begin === 0 &&
    pos.end === 0 &&
    isMask.call(inputmask, 0, true, true)
  ) {
    pos.end = 1;
  }
  ...
  if (maskset.wholeRegex) {
    ...
    const phChar = (i) =>
      typeof opts.placeholder === "string"
        ? opts.placeholder.charAt(i % opts.placeholder.length)
        : getPlaceholder.call(
            inputmask,
            i,
            getTestTemplate.call(inputmask, i).match
          );
    let filledValue = "";
...
function isValid(
    ...
    getTests.call(inputmask, position).every(function (tst, ndx) {
      ...
      if (
        ...
        if (test.displayChar !== undefined && c === test.displayChar) {
          ...
          const cached = inputmask._displayValueCache
            ? inputmask._displayValueCache[position]
            : undefined;
          rslt = {
            c:
              cached !== undefined && cached.input !== undefined
                ? cached.input
                : c,
            pos: position
          };
  ...
  if (
    ...
    result = opts.preValidation.call(
      ...
      maskset,
      pos,
      strict || typeof fromAlternate === "number"
    );
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            if (result !== false) {
              if (fromAlternate !== true && typeof fromAlternate !== "number")
...
function revalidateMask(pos, validTest, fromIsValid, validatedPos) {
  ...
  if (
    ...
    if (validTest && begin !== end) {
      const jitAnchor = Object.keys(maskset.jitOffset || {})
        .map((key) => parseInt(key, 10))
        .filter((key) => !isNaN(key))
        .sort((a, b) => a - b)[0];
      if (
        jitAnchor !== undefined &&
        maskset.jitOffset[jitAnchor] !== undefined
      ) {
