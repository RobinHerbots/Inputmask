...
function determineTestTemplate(pos, tests) {
  ...
  for (let ndx = 0; ndx < tests.length - lenghtOffset; ndx++) {
    ...
    const distance = Math.abs(tstLocator - targetLocator); // CHECK THIS ~ PLEASE why not just the abs from tstlocator?
        ...
        if (maskset.tests[pos] || maskset.validPositions[pos]) {
          (maskset.validPositions[pos]
            ...
            mlocMatches.every((mlocMatch) => {
              ...
              const mlocDistance = Math.abs(mlocMatchL);
        ...
        function mergeLoc(altNdx) {
          ...
          if (locNdx === undefined) {
            ...
            if (altMatch === undefined) {
              ...
              // select longest in targetMatch.mloc
              // const maxMloc = Object.values(targetMatch.mloc).sort(
              //   (a, b) => b.length - a.length
              // )[0];
              // // align the locator to the longest
              // for (let i = 0; i < maxMloc.length; i++) {
              //   if (
              //     targetMatch.locator[i] === undefined &&
              //     maxMloc[i].toString().indexOf(":") === -1
              //   ) {
              //     targetMatch.locator.push(maxMloc[i]);
              //   }
              // }

              targetMatch.locator[altNdx] = Object.values(targetMatch.mloc)
                .map((v) => v[altNdx])
