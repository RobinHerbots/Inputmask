	...
	function revalidateMask(pos, validTest, fromIsValid, validatedPos) {
		...
		if (begin !== end || (opts.insertMode && maskset.validPositions[validatedPos] !== undefined && fromIsValid === undefined)) {
			...
			var valid = true, j = validatedPos,
				needsValidation = validatedPos === 0, posMatch = j;
			i = j;
			...
			for (; i <= lvp; i++) {
				...
				if (t !== undefined && t.generatedInput !== true &&
					...
					while (getTest(posMatch).match.def !== "") { //loop needed to match further positions
						if (needsValidation === false && positionsClone[posMatch] && positionsClone[posMatch].match.nativeDef === t.match.nativeDef && t.generatedInput !== true) { //obvious match
