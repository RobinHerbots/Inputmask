...
module.exports = function maskScope(actionObj, maskset, opts) {
	...
	function revalidateMask(pos, validTest, fromIsValid, validatedPos) {
		...
		if (begin !== end || (opts.insertMode && maskset.validPositions[validatedPos] !== undefined && fromIsValid === undefined)) {
			...
			var positionsClone = $.extend(true, {}, maskset.validPositions),
				lvp = (validTest === undefined && opts.insertMode === false) ? pos.end - 1 : getLastValidPosition(undefined, true),
				i;
			...
			if (k === Inputmask.keyCode.BACKSPACE || k === Inputmask.keyCode.DELETE || (iphone && k === Inputmask.keyCode.BACKSPACE_SAFARI) || (e.ctrlKey && k === Inputmask.keyCode.X && !isInputEventSupported("cut"))) { //backspace/delete
				...
				if (opts.insertMode === false) {
					if (k === Inputmask.keyCode.RIGHT) {
						...
						setTimeout(function () {
							var caretPos = input.inputmask.caretPos;
	...
	function handleRemove(input, k, pos, strict, fromIsValid) {
		...
		if (lvp < pos.begin || pos.begin === -1) {
			...
			maskset.p = pos.begin;
			if (fromIsValid !== true && maskset.p !== 0) {
