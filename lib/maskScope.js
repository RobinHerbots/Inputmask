...
module.exports = function maskScope(actionObj, maskset, opts) {
	...
	function refreshFromBuffer(start, end, buffer) {
		...
		if (start === true) {
			...
			p = determineNewCaretPosition({begin: 0, end: 0}, false);
		} else {
			for (i = start; i < end; i++) {
		...
		inputFallBackEvent: function (e) { //fallback when keypress is not triggered
			...
			function analyseChanges(inputValue, buffer, caretPos) {
				...
				for (i = 0, bl = newBuffer.length; i < bl; i++) {
					...
					switch (action) {
						case "insertText":
							if (oldBuffer[i - 1] === newBuffer[i] && maskset.validPositions[0] === undefined) {
	...
	function checkVal(input, writeOut, strict, nptvl, initiatingEvent) {
		...
		if (writeOut) {
			...
			for (var vndx in maskset.validPositions) {
				delete maskset.validPositions[vndx].generatedInput; //clear generated markings
			}
		}
	}
