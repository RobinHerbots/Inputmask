...
module.exports = function maskScope(actionObj, maskset, opts) {
	...
	function refreshFromBuffer(start, end, buffer) {
		...
		for (i = start; i < end; i++) {
			...
			if (buffer[i] !== opts.skipOptionalPartCharacter) {
				var valResult = isValid(p, buffer[i], buffer[i] !== opts.negationSymbol.front, buffer[i] !== opts.negationSymbol.front);  //hackery for + validator (numeric alias)
				if (valResult !== false) {
					resetMaskSet(true);
	...
	function revalidateMask(pos, validTest, fromIsValid, validatedPos) {
		...
		if (begin !== end || (opts.insertMode && getMaskSet().validPositions[validatedPos] !== undefined && fromIsValid === undefined)) {
			...
			for (i = lvp; i >= begin; i--) {
				delete getMaskSet().validPositions[i];
				if (validTest === undefined) delete getMaskSet().tests[i + 1];
			}
