		...
		inputFallBackEvent: function (e) { //fallback when keypress is not triggered
			...
			function analyseChanges(inputValue, buffer, caretPos) {
				...
				var fpl = frontPart.length >= frontBufferPart.length ? frontPart.length : frontBufferPart.length,
					...
					bl, i, action = "", data = [], placeholder;

				//align buffers
				while (frontPart.length < fpl) {
					i = frontPart.length;
					frontPart.push(isMask(i, true) ? getPlaceholder(i) : "°");
				}
				;
				while (frontBufferPart.length < fpl) {
					i = frontBufferPart.length;
					frontBufferPart.push(isMask(i, true) ? getPlaceholder(i) : "°");
				}
				;
				while (backPart.length < bpl) {
					i = fpl + (bpl - backPart.length - 1);
					backPart.unshift(isMask(i, true) ? getPlaceholder(i) : "°");
				}
				;
				while (backBufferPart.length < bpl) {
					i = fpl + (bpl - backBufferPart.length - 1);
					backBufferPart.unshift(isMask(i, true) ? getPlaceholder(i) : "°");
				}
				;
				...
				for (i = 0, bl = newBuffer.length; i < bl; i++) {
					...
					switch (action) {
						case "insertText":
							if (oldBuffer[i] === placeholder && (newBuffer[i] !== placeholder || !isMask(i, true))) {
						...
						case "insertReplacementText":
							if (newBuffer[i] === placeholder) { //extend selection
								caretPos.end++;
							} else {
								// breakout loop
								i = bl;
						...
						default:
							if (newBuffer[i] !== oldBuffer[i]) {
								if (newBuffer[i] !== placeholder && oldBuffer[i] === placeholder && isMask(i)) {  //basic insert
									action = "insertText";
									data.push(newBuffer[i]);
									caretPos.begin--;
									caretPos.end--;
								} else if (oldBuffer[i + 1] === getPlaceholder(i + 1) && oldBuffer[i] === newBuffer[i + 1]) { //insert between
									action = "insertText";
									data.push(newBuffer[i]);
									caretPos.begin--;
									caretPos.end--;
								} else if (newBuffer[i] !== placeholder && newBuffer[i + 1] === getPlaceholder(i + 1)) { //replace selection
									action = "insertReplacementText";
									data.push(newBuffer[i]);
									caretPos.begin--;
								} else if (newBuffer[i] === placeholder && oldBuffer[i] !== placeholder) {  //delete~backspace
									action = "deleteContentForward";
									caretPos.end++;
								} else {
									i = bl;
								}
