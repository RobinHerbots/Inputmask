						...
						default:
							if (newBuffer[i] !== oldBuffer[i]) {
								if (newBuffer[i] !== placeholder && ((oldBuffer[i] === placeholder && oldBuffer[i + 1] === marker) || oldBuffer[i] === marker)) {  //basic insert
									...
									caretPos.begin--;
									caretPos.end--;
								} else if (newBuffer[i] !== placeholder && newBuffer[i] !== marker && newBuffer[i + 1] === marker) { //replace selection
									action = "insertReplacementText";
									data.push(newBuffer[i]);
									caretPos.begin--;
								} else if (newBuffer[i] === marker && (oldBuffer[i] !== placeholder || oldBuffer[i] === marker || !isMask(i, true))) {  //delete~backspace
									action = "deleteContentBackward";
									if (isMask(i, true)) caretPos.end++;
								} else {
									i = bl;
								}
							}
