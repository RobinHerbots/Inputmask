...
module.exports = function maskScope(actionObj, maskset, opts) {
	...
	function alternate(pos, c, strict, fromIsValid, rAltPos) { //pos == true => generalize
		...
		if (alternation !== undefined) {
			...
			while (maskset.excludes[decisionPos] !== undefined && maskset.excludes[decisionPos].length < 10) {
				...
				isValidRslt = true;
				insertPosition((pos + posOffset) === 0);
	...
	function revalidateMask(pos, validTest, fromIsValid, validatedPos) {
		function IsEnclosedStatic(pos, valids, selection) {
			...
			if (posMatch !== undefined && ((posMatch.match.static === true && posMatch.match.optionality !== true && posMatch.generatedInput) /*|| posMatch.input === opts.radixPoint*/)) {
		...
		if (begin !== end || (opts.insertMode && maskset.validPositions[validatedPos] !== undefined && fromIsValid === undefined) || validTest === undefined) {
			...
			if (validTest || opts.insertMode) {
				for (; i <= lvp; i++) {
					...
					if (t !== undefined && t.generatedInput !== true &&
	...
	var EventHandlers = {
		keydownEvent: function (e, fromInputFallback) {
			...
			if (k === Inputmask.keyCode.BACKSPACE || k === Inputmask.keyCode.DELETE || (iphone && k === Inputmask.keyCode.BACKSPACE_SAFARI) || (e.ctrlKey && k === Inputmask.keyCode.X && !isInputEventSupported("cut"))) { //backspace/delete
				...
				} else if (opts.colorMask === true) {
					if (k === Inputmask.keyCode.RIGHT || k === Inputmask.keyCode.LEFT) {
						setTimeout(function () {
							var caretPos = caret(input, undefined, undefined, true);
	...
	function initializeColorMask(input) {
		...
		$(colorMask).on("click", function (e) {
			caret(input, findCaretPos(e.clientX), undefined, true);
