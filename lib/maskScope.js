			...
			for (; i <= lvp; i++) {
				...
				if (t !== undefined && t.generatedInput !== true &&
					...
					while (getTest(posMatch).match.def !== "") { //loop needed to match further positions
						if (opts.shiftPositions === false && posMatch === validatedPos && !validTest) {  //delete
							valid = false;
						} else if (positionCanMatchDefinition(posMatch, t.match.def) || t.match.def === "+") { //validated match //we still need some hackery for the + validator (numeric alias)
