  ...
  inputValue.forEach(function (charCode, ndx) {
    if (charCode !== undefined) {
      ...
      if (result) {
        ...
        const onbeforeWriteResult = writeBuffer.call(
          inputmask,
          undefined,
...
function writeBuffer(input, buffer, caretPos, event, triggerEvents) {
  ...
  if (event && typeof opts.onBeforeWrite === "function") {
    ...
    onBeforeWriteResult = opts.onBeforeWrite.call(
      inputmask,
      event,
      buffer,
      caretPos,
    ...
    if (onBeforeWriteResult) {
      if (onBeforeWriteResult.refreshFromBuffer) {
        ...
        refreshFromBuffer.call(
          ...
          refresh.end,
          onBeforeWriteResult.buffer || buffer
        );
        buffer = getBuffer.call(inputmask, true);
      }
      if (caretPos !== undefined)
        caretPos =
          onBeforeWriteResult.caret !== undefined
            ? onBeforeWriteResult.caret
            : caretPos;
    }
  }
  if (input !== undefined) {
