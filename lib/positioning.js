		...
		switch (opts.positionCaretOnClick) {
			...
			default: //lvp:
				var clickPosition = selectedCaret.begin,
					...
					lastPosition = seekNext.call(inputmask, (lvclickPosition === -1 && !isMask.call(inputmask, 0)) ? 0 : lvclickPosition);
				if (clickPosition < lastPosition) {
					selectedCaret.end = selectedCaret.begin = !isMask.call(inputmask, clickPosition, true) && !isMask.call(inputmask, clickPosition - 1, true) ? seekNext.call(inputmask, clickPosition) : clickPosition;
				} else {
					...
					if ((placeholder !== "" && getBuffer.call(inputmask)[lastPosition] !== placeholder && tt.match.optionalQuantifier !== true && tt.match.newBlockMarker !== true) || (!isMask.call(inputmask, lastPosition, opts.keepStatic) && tt.match.def === placeholder)) {
...
function seekPrevious(pos, newBlock) {
	...
	while (--position > 0 &&
	((newBlock === true && getTest.call(inputmask, position).match.newBlockMarker !== true) ||
		(newBlock !== true && !isMask.call(inputmask, position, undefined, true) &&
			// eslint-disable-next-line no-empty
			(tests = getTests.call(inputmask, position), tests.length < 2 || (tests.length === 2 && tests[1].match.def === ""))))) {
	}
	return position;
}
