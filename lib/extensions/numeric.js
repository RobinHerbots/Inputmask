...
function decimalValidator(chrs, maskset, pos, strict, opts) {
  const radixPos = maskset.buffer
      ...
      : -1,
    result = (radixPos !== -1 || (strict && opts.jitMasking)) && isDigit(chrs);
  if (
    !strict &&
    opts._radixDance &&
    radixPos !== -1 &&
...
const numericAlias = {
    ...
    preValidation: function (
      ...
      // With jitMasking the integer part isn't rendered yet, so the placeholder
      // zero the full mask shows is missing: typing the radix or a suffix char
      // as the first entry leaves ".5" / "%" where the full mask has "0.5" /
      // "0%". Insert the zero explicitly. Fixes #2561.
      if (
        opts.jitMasking !== false &&
        opts.jitMasking !== undefined &&
        !isDigit(c) &&
        (c === opts.radixPoint || opts.suffix.indexOf(c) !== -1) &&
        radixPos === -1 &&
        isSelection === false &&
        !maskset.validPositions.some(
          (vp) => vp && !vp.generatedInput && isDigit(vp.input)
        )
      ) {
        // the buffer is reversed, so the digits start behind the suffix and
        // behind the closing negation symbol when the value is already negative
        const backNdx = findValid("-", maskset),
          tailLength =
            opts.suffix.length +
            (backNdx !== -1 ? opts.negationSymbol.back.length : 0),
          integerPos =
            backNdx !== -1
              ? backNdx + opts.negationSymbol.back.length
              : findValidator.call(inputmask, "9", maskset);
        return {
          insert: [{ pos: integerPos, c: "0", fromIsValid: true }],
          rewritePosition: c === opts.radixPoint ? tailLength : pos
        };
      }
      if (
        radixPos !== -1 &&
        ...
        switch (e.type) {
          ...
          case "keydown":
            if (
              ...
              if (
                buffer[
                  e.key === keys.Delete ? caretPos.begin - 1 : caretPos.end
                ] === opts.negationSymbol.front ||
                (buffer.length - _buffer.length ===
                  opts.negationSymbol.front.length +
                    opts.negationSymbol.back.length &&
                  buffer.join("").indexOf(_buffer.join("")) >= 0)
              ) {
                result = {
                  refreshFromBuffer: true,
                  buffer: [],
