...
function prefillYear(dateParts, currentResult, opts) {
	...
	if (dateParts.year !== dateParts.rawyear) {
		...
		if (enteredPart.length === 2 && enteredPart === currentYearPart && (!opts.max || opts.max.year >= crrntyear)) {
			//update dateParts
...
function isDateInRange(dateParts, result, opts, maskset) {
	...
	if (opts.min) {
		if (dateParts["rawyear"]) {
			...
			if (rawYear < minYear) { //is out of range?
				...
				if (minYear <= rawYear) { //this can match
					...
					rawYear = dateParts["rawyear"].substr(0, (result.pos - tokenMatch.targetMatchIndex));
					minYear = opts.min.year.substr(2, 1);
					if (rawYear.length === 1 && minYear <= rawYear) { //this can match
						result.insert = [{
		...
		if (result && dateParts["year"] === dateParts["rawyear"]) {
...
Inputmask.extendAliases({
	"datetime": {
		mask: function (opts) {
			...
			opts.min = analyseMask(opts.min || importDate(new Date(currentYear - 2, 0, 1), opts), opts.inputFormat, opts);
		...
		postValidation: function (buffer, pos, c, currentResult, opts, maskset, strict) {
			...
			if (result && dateParts.date.getTime() === dateParts.date.getTime()) { //check for a valid date ~ an invalid date returns NaN which isn't equal
				...
				result = isDateInRange(dateParts, result, opts, maskset);
			}
