...
const datetimeAlias = {
  ...
  onBeforeMask: function (initialValue, opts) {
    if (Object.prototype.toString.call(initialValue) === "[object Date]") {
      ...
      const matches = (dateParts, format) =>
          dateParts !== undefined &&
          !isNaN(dateParts.date.getTime()) &&
          parse(format, dateParts, opts) === initialValue,
      ...
      if (!matches(inFormat, opts.inputFormat)) {
        ...
        if (matches(outFormat, opts.outputFormat))
          initialValue = importDate(outFormat.date, opts);
      }
    }
