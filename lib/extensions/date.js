...
import Inputmask, { aliases } from "../inputmask";
import { keys } from "../keycode.js";
import { getBuffer, seekNext } from "../positioning";
...
function parse(format, dateObjValue, opts, partialFields) {
      ...
      if (dateObjValue === undefined) {
        ...
        if (!escaped && (fcode = formatcode(match[0]))) {
          if (fcode[3]) {
            const getFn = fcode[3];
            mask +=
              partialFields && partialFields[fcode[2]]
                ? match[0]
                : getFn.call(dateObjValue.date);
...
// maps the date fields that are not completely filled in the masked value
function getPartialFields(inputmask, mask, format, opts) {
  const fields = {};
  let lastNdx = -1;
  ...
  while ((match = tokenizer.exec(format))) {
    if (match.index >= lastNdx) {
      ...
      let value;
      if (dynMatches) {
        const lastIndex = tokenizer.lastIndex,
          tokenMatch = getTokenMatch.call(
            inputmask,
            match.index,
            opts,
      ...
      mask = mask.slice(value.length);
      const fcodeInfo = formatcode(fcode);
      if (fcodeInfo && fcodeInfo[2]) {
        if (
          value === undefined ||
          (/[^0-9]/.test(value) &&
...
export { datetime, registerDatetime };
...
const datetimeAlias = {
  mask: function (opts) {
    ...
    opts.displayFormat =
      formatAlias[opts.displayFormat] || opts.displayFormat || opts.inputFormat; // resolve possible formatAlias
    opts.outputFormat =
  ...
  postValidation: function (
    ...
    if (currentResult === false) {
      ...
      if (
        tokenMatch.targetMatch &&
        tokenMatch.targetMatchIndex === pos.begin &&
        tokenMatch.targetMatch[0].length > 1 &&
        ...
        if (
          tokenMatch.targetMatch &&
          tokenMatch.targetMatchIndex === pos.begin + 1 &&
          tokenMatch.targetMatch[0].length > 1 &&
          formatcode(tokenMatch.targetMatch[0]) !== undefined
        ) {
      ...
      if (validator !== undefined) {
        ...
        if (
          maskset.validPositions[pos.begin + 1] !== undefined &&
          ...
          if (selectionReplace > 0) {
            ...
            buffer.splice(
              pos.begin,
              selectionReplace,
              ..._buffer.slice(pos.begin, pos.begin + selectionReplace)
            );
            for (let i = pos.begin; i < pos.begin + selectionReplace; i++)
    ...
    if (currentResult.fuzzy) {
      ...
      pos = { begin: currentResult.pos, end: currentResult.pos };
    }
    ...
    if (
      ...
      if (
        ...
        if (pos.begin === tokenMatch.targetMatchIndex) {
          ...
          const tpl = getMaskTemplate.call(
            inputmask,
            false,
            1,
            undefined,
          ...
          buffer[tokenMatch.targetMatchIndex + 1] =
            tpl[tokenMatch.targetMatchIndex + 1];
        } else {
    ...
    if (pos !== undefined && result && currentResult.pos !== pos.begin) {
      return {
        ...
        refreshFromBuffer: { start: pos.begin, end: currentResult.pos },
        pos:
  ...
  onUnMask: function (maskedValue, unmaskedValue, opts) {
    const inputmask = this;
    // a partial date (placeholders still present in the buffer) must not be
    // composed into a fabricated value (e.g. year 0000) - compose it in the
    // outputFormat keeping the placeholders for the missing fields
    if (
      unmaskedValue &&
      isComplete.call(inputmask, getBuffer.call(inputmask, true)) !== true
    ) {
      return parse(
        opts.outputFormat,
        analyseMask.call(inputmask, maskedValue, opts.inputFormat, opts),
        opts,
        getPartialFields(inputmask, maskedValue, opts.inputFormat, opts)
      );
    }
...
function datetime(options) {
  return $.extend(true, {}, datetimeAlias, options);
}
