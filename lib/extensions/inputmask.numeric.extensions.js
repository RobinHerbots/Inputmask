...
function checkForLeadingZeroes(buffer, opts, strict) {
	...
	return leadingzeroes && (leadingzeroes[0].length > strict ? 0 : 1 || leadingzeroes[0].length > 0 && leadingzeroes[0].length < number.length) ? leadingzeroes : false;
}
...
Inputmask.extendAliases({
	"numeric": {
		...
		preValidation: function (buffer, pos, c, isSelection, opts, maskset, caretPos) {
			...
			return {rewritePosition: isSelection ? (checkForLeadingZeroes(buffer, opts, true) ? radixPos : caretPos.begin - 1) : pos};
		},
