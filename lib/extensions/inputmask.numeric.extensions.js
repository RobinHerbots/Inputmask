...
Inputmask.extendAliases({
	"numeric": {
		...
		onBeforeWrite: function (e, buffer, caretPos, opts) {
			//check leading zeros
			var numberMatches = new RegExp("^" + Inputmask.escapeRegex(opts.prefix) + "(?<number>.*)" + Inputmask.escapeRegex(opts.suffix) + "$").exec(buffer.slice().reverse().join("")),
				number = numberMatches.groups.number;
			number = number.split(opts.radixPoint.charAt(0))[0];

			var leadingzeroes = new RegExp("^[0" + opts.groupSeparator + "]*").exec(number);

			if (leadingzeroes[0].length > 0) {
