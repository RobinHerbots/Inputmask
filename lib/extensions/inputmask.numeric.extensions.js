	...
	for (var tstNdx in maskset.tests) {
		...
		if (tstNdx >= posNdx) {
			for (var ndx = 0, ndxl = maskset.tests[tstNdx].length; ndx < ndxl; ndx++) {
				if (maskset.validPositions[tstNdx] === undefined && maskset.tests[tstNdx][ndx].match.def === "+") {
	...
	"numeric": {
		...
		onBeforeWrite: function (e, buffer, caretPos, opts) {
			...
			if (leadingzeroes[0].length > 1 || leadingzeroes[0].length > 0 && leadingzeroes[0].length < number.length) {
				...
				return {refreshFromBuffer: true, buffer: buf.reverse(), caret: buf.length - caretNdx};
			}
		},
