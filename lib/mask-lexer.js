...
const tokenizer =
  ...
  regexTokenizer =
    /\[\^?]?(?:[^\\\]]+|\\[\S\s]?)*]?|\\(?:0(?:[0-3][0-7]{0,2}|[4-7][0-7]?)?|[1-9][0-9]*|x[0-9A-Fa-f]{2}|u[0-9A-Fa-f]{4}|c[A-Za-z]|[\S\s]?)|\((?:\?(?:[:=!]|<[\w$][^>]*>|<[=!])?)?|(?:[?*+]|\{[0-9]+(?:,[0-9]*)?\})\??|[^.?*+^${[()|\\]+|./g;
  ...
  function generateMask(mask, metadata, opts) {
    ...
    if (masksCache[maskdefKey] === undefined || nocache === true) {
      ...
      if (regexMask) {
        ...
        try {
          masksetDefinition.wholeRegex = new RegExp(
            "^(?:" + opts.regex + ")$",
            "u"
          );
        } catch (e) {
...
function analyseMask(mask, regexMask, opts) {
  ...
  function insertTestDefinition(mtoken, element, position) {
    ...
    if (regexMask) {
      if (
        element.indexOf("[") === 0 ||
        (escaped &&
          /\\(?:[dDsSwWpPnrtfv]|0(?:[0-3][0-7]{0,2}|[4-7][0-7]?)?|x[0-9A-Fa-f]{2}|u[0-9A-Fa-f]{4}|c[A-Za-z])/.test(
            element
          )) ||
        element === "."
      ) {
  ...
  function skipAssertion() {
    ...
    do {
      ...
      if (assertionToken[0].charAt(0) === "(") depth++;
      else if (assertionToken[0] === ")") depth--;
    } while (depth > 0);
    if (depth === 0) {
      ...
      case opts.quantifiermarker[0]:
        {
          ...
          if (
            isNaN(quantifier.quantifier.min) &&
            isNaN(quantifier.quantifier.max)
          ) {
