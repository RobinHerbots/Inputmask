...
e,l(),g);Y=-1!=f.inArray(h,g.ignorables)}function da(h,m,x,$,n,B){if(void 0==x&&T)return!1;T=!0;var z=f(this),h=h||window.event,x=x||h.which||h.charCode||h.keyCode,t=String.fromCharCode(x);if(g.numericInput&&t==g.radixPoint){var E=this._valueGet().indexOf(g.radixPoint);v(this,r(-1!=E?E:p()))}if((h.ctrlKey||h.metaKey||Y)&&!0!==m)return!0;if(x){var C,N;m?(x=n?B:g.numericInput?r(c().p):c().p,C={begin:x,end:x}):C=v(this);var x=b,H=!1;f.each(a,function(a){b=a;c().undoBuffer=l().join("");if(C.end-C.begin>
1||C.end-C.begin==1&&g.insertMode){a=C.end<p()?C.end:p();O(l(),C.begin,a);var e=p();if(g.greedy==false)o?y(0,a-1,k(a),true):K(C.begin,e);else for(var d=C.begin;d<a;d++)s(d)&&(o?y(0,a-1,k(a),true):K(C.begin,e));H=true}});b=x;if(o){var u=D(H?C.begin:C.end),x=j(u,t,n,o);!0===n&&(x=[{activeMasksetIndex:b,result:x}]);f.each(x,function(a,e){b=e.activeMasksetIndex;c().writeOutBuffer=true;var d=e.result;if(d!==false){var h=false,f=l();if(d!==true){h=d.refresh;u=d.pos!=void 0?d.pos:u;t=d.c!=void 0?d.c:t}if(h!==
true){var h=p(),j=r(-1),d=j;if(g.insertMode==true){if(c().greedy==true)for(var n=f.slice();G(n,d,true)!=k(d)&&d<=u;)d=d==h?h+1:r(d);if(d<=u&&(c().greedy||f.length<h||G(f,u)==k(u))){if(f[j]!=k(j)&&f.length<h){f=F(f,-1,o);if((H?C.begin:C.end)!=0)u=u+f}K(d,u,t)}else c().writeOutBuffer=false}else w(f,u,t,true,o)}c().p=u}});!0!==n&&A(o);if(!1!==$&&(f.each(x,function(a,c){if(c.activeMasksetIndex==b){N=c;return false}}),void 0!=N)){var M=this;setTimeout(function(){g.onKeyValidation.call(M,N.result,g)},0);
if(c().writeOutBuffer&&!1!==N.result){var Q=l();L(this,Q,m?void 0:g.numericInput?r(c().p):c().p);setTimeout(function(){P(Q)&&z.trigger("complete")},0)}else c().buffer=c().undoBuffer.split("")}}else u=r(C.begin-1),x=j(u,t,n,o),!0===n&&(x=[{activeMasksetIndex:b,result:x}]),f.each(x,function(a,e){b=e.activeMasksetIndex;c().writeOutBuffer=true;var d=e.result;if(d!==false){var h=false,f=l();if(d!==true){h=d.refresh;u=d.pos!=void 0?d.pos:u;t=d.c!=void 0?d.c:t}if(h!==true)if(g.insertMode==true){d=p();for(h=
f.slice();G(h,d,true)!=k(d)&&d>=u;)d=d==0?-1:D(d);d>=u?y(u,f.length,t):c().writeOutBuffer=false}else w(f,u,t,true,o);c().p=r(u)}}),!0!==n&&A(o),!1!==$&&(f.each(x,function(a,c){if(c.activeMasksetIndex==b){N=c;return false}}),void 0!=N&&(M=this,setTimeout(function(){g.onKeyValidation.call(M,N.result,g)},0),c().writeOutBuffer&&!1!==N.result?(u=c().p,Q=l(),L(this,Q,m?void 0:u),setTimeout(function(){P(Q)&&z.trigger("complete")},0)):c().buffer=c().undoBuffer.split("")));S&&!0!==m&&v(this,e,d);g.showTooltip&&
z.prop("title",c().mask);h.preventDefault()}}function ba(a){var b=f(this),d=a.keyCode,e=l();g.onKeyUp.call(this,a,e,g);d==g.keyCode.TAB&&(b.hasClass("focus.inputmask")&&0==this._valueGet().length&&g.showMaskOnFocus)&&(e=m().slice(),L(this,e),o||v(this,0),c().undoBuffer=this._valueGet())}var z=f(B);if(z.is(":input")){z.data("inputmask",{masksets:a,activeMasksetIndex:b,opts:g,isRTL:!1});g.showTooltip&&z.prop("title",c().mask);c().greedy=c().greedy?c().greedy:0==c().repeat;var E=z.prop("maxLength");
p()>E&&-1<E&&(E<m().length&&(m().length=E),!1==c().greedy&&(c().repeat=Math.round(E/m().length)),z.prop("maxLength",2*p()));W(B);c().undoBuffer=B._valueGet();var T=!1,Y=!1,o=!1;if("rtl"==B.dir||g.numericInput)("rtl"==B.dir||g.numericInput&&g.rightAlignNumerics)&&z.css("text-align","right"),B.dir="ltr",z.removeAttr("dir"),E=z.data("inputmask"),E.isRTL=!0,z.data("inputmask",E),o=!0;z.unbind(".inputmask");z.removeClass("focus.inputmask");z.bind("mouseenter.inputmask",function(){!f(this).hasClass("focus.inputmask")&&
...
"return;"),c="function"==typeof b[a]);return c}("paste")&&!F?"paste":"input",aa,A,w=0;S&&(F=navigator.userAgent.match(/safari.*/i),aa=533>=parseInt(RegExp(/[0-9]+/).exec(F)));if("string"===typeof a)switch(a){case "mask":return e(g.alias,d),A=h(),this.each(function(){j(f.extend(true,{},A),0).mask(this)});case "unmaskedvalue":return F=f(this),A=F.data("inputmask").masksets,w=F.data("inputmask").activeMasksetIndex,g=F.data("inputmask").opts,j(A,w).unmaskedvalue(this);case "remove":return this.each(function(){var a=
f(this),b=this;setTimeout(function(){if(a.data("inputmask")){A=a.data("inputmask").masksets;w=a.data("inputmask").activeMasksetIndex;g=a.data("inputmask").opts;b._valueSet(j(A,w).unmaskedvalue(a,true));a.removeData("inputmask");a.unbind(".inputmask");a.removeClass("focus.inputmask");var c;Object.getOwnPropertyDescriptor&&(c=Object.getOwnPropertyDescriptor(b,"value"));if(c&&c.get)b._valueGet&&Object.defineProperty(b,"value",{get:b._valueGet,set:b._valueSet});else if(document.__lookupGetter__&&b.__lookupGetter__("value")&&
b._valueGet){b.__defineGetter__("value",b._valueGet);b.__defineSetter__("value",b._valueSet)}delete b._valueGet;delete b._valueSet}},0)});case "getemptymask":return this.data("inputmask")?(A=this.data("inputmask").masksets,w=this.data("inputmask").activeMasksetIndex,A[w]._buffer.join("")):"";case "hasMaskedValue":return this.data("inputmask")?!this.data("inputmask").opts.autoUnmask:!1;case "isComplete":return A=this.data("inputmask").masksets,w=this.data("inputmask").activeMasksetIndex,g=this.data("inputmask").opts,
j(A,w).isComplete(this[0]._valueGet().split(""));default:return e(a,d)||(g.mask=a),A=h(),this.each(function(){j(f.extend(true,{},A),w).mask(this)})}else{if("object"==typeof a)return g=f.extend(!0,{},f.inputmask.defaults,a),e(g.alias,a),A=h(),this.each(function(){j(f.extend(true,{},A),w).mask(this)});if(void 0==a)return this.each(function(){var a=f(this).attr("data-inputmask");if(a&&a!="")try{var a=a.replace(RegExp("'","g"),'"'),b=f.parseJSON("{"+a+"}");f.extend(true,b,d);g=f.extend(true,{},f.inputmask.defaults,
b);e(g.alias,b);g.alias=void 0;f(this).inputmask(g)}catch(c){}})}return this})})(jQuery);
