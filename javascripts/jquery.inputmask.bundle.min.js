...
this.lastValidPosition});b=h.activeMasksetIndex;d!=b&&(e?M(l(),0,E(h.lastValidPosition)):M(l(),s(h.lastValidPosition),p()),c().writeOutBuffer=!0)}function r(a){a=u(a);a=h()[a];return void 0!=a?a.fn:!1}function u(a){return a%h().length}function p(){return g.getMaskLength(m(),c().greedy,c().repeat,l(),g)}function s(a){var b=p();if(a>=b)return b;for(;++a<b&&!r(a););return a}function E(a){if(0>=a)return 0;for(;0<--a&&!r(a););return a}function v(a,b,c,e,d){e&&(b=F(a,b,d));e=h()[u(b)];d=c;if(void 0!=d)switch(e.casing){case "upper":d=
c.toUpperCase();break;case "lower":d=c.toLowerCase()}a[b]=d}function G(a,b,c){c&&(b=F(a,b));return a[b]}function F(a,b,c){if(c)for(;0>b&&a.length<p();){c=m().length-1;for(b=m().length;void 0!==m()[c];)a.unshift(m()[c--])}else for(;void 0==a[b]&&a.length<p();)for(c=0;void 0!==m()[c];)a.push(m()[c++]);return b}function L(a,b,c){a._valueSet(b.join(""));void 0!=c&&w(a,c)}function M(a,b,c){for(var e=p();b<c&&b<e;b++)v(a,b,G(m().slice(),b,!0))}function P(a,b){var c=u(b);v(a,b,G(m(),c))}function H(e,d,h,
...
end:0};h.setSelectionRange?(b=h.selectionStart,c=h.selectionEnd):document.selection&&document.selection.createRange&&(a=document.selection.createRange(),b=0-a.duplicate().moveStart("character",-1E5),c=b+a.text.length);return{begin:b,end:c}}}function N(c){var e=!1,d=0,h=b;f.each(a,function(a,h){b=a;var f=E(p());if(void 0!=h.lastValidPosition&&h.lastValidPosition>=d&&h.lastValidPosition==f){for(var g=!0,l=0;l<=f;l++){var j=r(l),I=u(l);if(j&&(void 0==c[l]||c[l]==k(l))||!j&&c[l]!=m()[I]){g=!1;break}}if(e=
...
function W(a,b){if(g.numericInput&&""!=g.radixPoint&&!1===g.skipRadixDance){var c=a._valueGet().indexOf(g.radixPoint);o=b.begin<=c||b.end<=c||-1==c}}function K(a,b,e){for(var d=l();!r(a)&&0<=a-1;)a--;for(var f=a;f<b&&f<p();f++)if(r(f)){P(d,f);var g=s(f),I=G(d,g);if(I!=k(g))if(g<p()&&!1!==j(f,I,!0,o)&&h()[u(f)].def==h()[u(g)].def)v(d,f,G(d,g),!0,o),g<b&&P(d,g);else if(r(f))break}else P(d,f);void 0!=e&&v(d,o?b:E(b),e);if(!1==c().greedy){b=T(d.join(""),o).split("");d.length=b.length;f=0;for(e=d.length;f<
e;f++)d[f]=b[f];0==d.length&&(c().buffer=m().slice())}return a}function y(a,b,e,d){for(var f=l();a<=b&&a<p();a++)if(r(a)){var g=G(f,a);v(f,a,e,!0,o);if(g!=k(a))if(e=s(a),e<p())if(!1!==j(e,g,!0,o)&&h()[u(a)].def==h()[u(e)].def)e=g;else if(r(e))break;else e=g;else break;else if(e=g,!0!==d)break}else P(f,a);d=f.length;if(!1==c().greedy){e=T(f.join(""),o).split("");f.length=e.length;a=0;for(g=f.length;a<g;a++)f[a]=e[a];0==f.length&&(c().buffer=m().slice())}return b-(d-f.length)}function ba(e){S=!1;var d=
this,h=e.keyCode,j=w(d);W(d,j);if(h==g.keyCode.BACKSPACE||h==g.keyCode.DELETE||da&&127==h||e.ctrlKey&&88==h){e.preventDefault();var n=j.begin;if(0==j.begin&&j.end==p())M(l(),j.begin,j.end),f.each(a,function(a,b){b.buffer=b._buffer.slice();b.lastValidPosition=void 0;b.p=o?p():0});else if(1<j.end-j.begin||1==j.end-j.begin&&g.insertMode){M(l(),j.begin,j.end);var q=p();if(!1==g.greedy)o?y(0,j.end-1,k(j.end),!0):K(j.begin,q);else for(var v=j.begin;v<j.end;v++)r(v)&&(o?y(0,j.end-1,k(j.end),!0):K(j.begin,
...
k(d)&&d<=t;)d=d==h?h+1:s(d);if(d<=t&&(c().greedy||f.length<h||G(f,t)==k(t))){if(f[j]!=k(j)&&f.length<h){f=F(f,-1,o);if((H?D.begin:D.end)!=0)t=t+f}K(d,t,u)}else c().writeOutBuffer=false}else v(f,t,u,true,o)}c().p=t}});!0!==n&&A(o);if(!1!==Z&&(f.each(x,function(a,c){if(c.activeMasksetIndex==b){C=c;return false}}),void 0!=C))if(setTimeout(function(){g.onKeyValidation.call(this,C.result,g)},0),c().writeOutBuffer&&!1!==C.result){var O=l();L(this,O,m?void 0:g.numericInput?s(c().p):c().p);setTimeout(function(){N(O)&&
...
s(t)}}),!0!==n&&A(o),!1!==Z&&(f.each(x,function(a,c){if(c.activeMasksetIndex==b){C=c;return false}}),void 0!=C&&(setTimeout(function(){g.onKeyValidation.call(this,C.result,g)},0),c().writeOutBuffer&&!1!==C.result?(t=c().p,O=l(),L(this,O,m?void 0:t),setTimeout(function(){N(O)&&z.trigger("complete")},0)):c().buffer=c().undoBuffer.split("")));R&&!0!==m&&w(this,e,d);g.showTooltip&&z.prop("title",c().mask);h.preventDefault()}}function aa(a){var b=f(this),d=a.keyCode,e=l();g.onKeyUp.call(this,a,e,g);d==
g.keyCode.TAB&&(b.hasClass("focus.inputmask")&&0==this._valueGet().length&&g.showMaskOnFocus)&&(e=m().slice(),L(this,e),o||w(this,0),c().undoBuffer=this._valueGet())}var z=f(B);if(z.is(":input")){z.data("inputmask",{masksets:a,activeMasksetIndex:b,opts:g,isRTL:!1});g.showTooltip&&z.prop("title",c().mask);c().greedy=c().greedy?c().greedy:0==c().repeat;var C=z.prop("maxLength");p()>C&&-1<C&&(C<m().length&&(m().length=C),!1==c().greedy&&(c().repeat=Math.round(C/m().length)),z.prop("maxLength",2*p()));
